...
const Workouts = () => {
  ...
  const totalPages = Math.ceil(filteredExercises.length ? filteredExercises.length / itemsPerPage : filteredWorkouts.length / itemsPerPage);
  ...
  const handleAddToFavorites = (workout) => {
    alert(`Added "${workout.name}" to favorites!`);
  };
  const handleShareWorkout = (workout) => {
    alert(`Shared "${workout.name}"!`);
  };
      ...
      {selectedWorkout && (
        ...
        >
          ...
          >
              ...
              <div>
                ...
                <ol>
                  {selectedWorkout.instructions.map((step, idx) => (
                    <li key={idx} style={{ textDecoration: checkedSteps[idx] ? "line-through" : "none" }}>
                      <input
                        type="checkbox"
            ...
            <div>
              <h4>User Reviews:</h4>
              {exerciseReviews[selectedWorkout.id] ? (
                <ul>
                  {exerciseReviews[selectedWorkout.id].map((rev, idx) => (
                    <li key={idx}>
            ...
            >
              <button
                onClick={() => handleToggleFavorite(selectedWorkout)}
                style={{
                  padding: "0.5rem 1rem",
